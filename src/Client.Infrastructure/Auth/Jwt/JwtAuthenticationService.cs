...
public class JwtAuthenticationService : AuthenticationStateProvider, IAuthenticationService, IAccessTokenProvider
{
    ...
    public async ValueTask<AccessTokenResult> RequestAccessToken()
    {
        ...
        {
            return new AccessTokenResult(AccessTokenResultStatus.RequiresRedirect, new AccessToken(), "/login", new InteractiveRequestOptions { Interaction = InteractionType.SignIn, ReturnUrl = "/login" });
        }
        ...
        try
        {
            ...
            return new AccessTokenResult(AccessTokenResultStatus.Success, new AccessToken() { Value = token ?? string.Empty }, string.Empty, new InteractiveRequestOptions { Interaction = InteractionType.GetToken, ReturnUrl = string.Empty });
        }
        finally
        {
            _semaphore.Release();
